...
class AudioPlayerBloc extends Bloc<AudioPlayerEvent, AudioPlayerState> {
  ...
  StreamSubscription? _currentMediaSubscription;
  ...
  void _initializeListeners() {
    ...
    _stateSubscription = _playerService.playerStateStream.listen((playerState) {
      if (!isClosed) {
        emit(state.copyWith(
    ...
    _queueSubscription = _playerService.queueStream.listen((queue) {
      if (!isClosed) {
        emit(state.copyWith(queue: queue));
      }
    });

    _currentIndexSubscription = _playerService.queueIndexStream.listen((index) {
      if (!isClosed) {
        emit(state.copyWith(currentIndex: index));
        _logger.debug('🔄 Current index updated to: $index');
      }
    });

    _currentMediaSubscription = _playerService.currentMediaStream.listen((media) {
      if (!isClosed) {
        emit(state.copyWith(currentMedia: media));
        _logger.debug('🔄 Current media updated to: ${media?.title ?? 'null'}');
      }
    });
  }
  ...
  Future<void> _onPlayMedia(PlayMedia event, Emitter<AudioPlayerState> emit) async {
    try {
      ...
      // The listeners will update the state
    } catch (e) {
  ...
  Future<void> _onNextTrack(NextTrack event, Emitter<AudioPlayerState> emit) async {
    try {
      _logger.info('➡️ NextTrack event received');
  ...
  Future<void> _onPreviousTrack(PreviousTrack event, Emitter<AudioPlayerState> emit) async {
    try {
      _logger.info('⬅️ PreviousTrack event received');
